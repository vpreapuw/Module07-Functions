...
-- 2024-05-28,Vugee Preap,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_VugeePreap')
	 Begin 
	  Alter Database [Assignment07DB_VugeePreap] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_VugeePreap;
	 End
	Create Database Assignment07DB_VugeePreap;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_VugeePreap;
...
SELECT 
    c.CategoryName, 
    p.ProductName, 
    FORMAT(p.Price, 'C', 'en-US') AS FormattedPrice
FROM 
    Categories c
JOIN 
...
SELECT 
    c.CategoryName, 
    FORMAT(i.InventoryDate, 'MMMM, yyyy') AS FormattedDate, 
    SUM(i.InventoryCount) AS TotalInventoryCount
FROM 
    Categories c
JOIN 
    Products p ON c.CategoryID = p.CategoryID
JOIN 
    Inventories i ON p.ProductID = i.ProductID
GROUP BY 
    c.CategoryName, FORMAT(i.InventoryDate, 'MMMM, yyyy')
ORDER BY 
    c.CategoryName, FORMAT(i.InventoryDate, 'MMMM, yyyy');
go
